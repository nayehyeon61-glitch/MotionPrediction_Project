...
M.thesaurus={
	...
	LEFTHIP= {'lefthip', 'lhip', 'lfemur', 'thigh_l'}, -- like a thesaurus.
	RIGHTHIP= {'righthip', 'rhip', 'rfemur', 'thigh_r'},
	LEFTKNEE={'leftknee', 'lknee', 'ltibia', 'calf_l'},
	RIGHTKNEE={'rightknee', 'rknee', 'rtibia', 'calf_r'},
	LEFTANKLE={'leftankle', 'lankle','lfoot', 'foot_l' },
	RIGHTANKLE={'rightankle', 'rankle', 'rfoot', 'foot_r'},
	LEFTSHOULDER= {'leftshoulder', 'lshoulder','lhumerus', 'upperarm_l'},
	RIGHTSHOULDER= {'rightshoulder', 'rshoulder','rhumerus', 'upperarm_r'},
	LEFTELBOW= {'leftelbow', 'lelbow', 'lradius', 'lowerarm_l'},
	RIGHTELBOW= {'rightelbow', 'relbow', 'rradius', 'lowerarm_r'},
	LEFTWRIST= {'leftwrist', 'lwrist', 'lhand', 'hand_l'},
	RIGHTWRIST= {'rightwrist', 'rwrist', 'rhand', 'hand_r'},
	LEFTCOLLAR={'leftcollar', 'lcollar', 'lclavicle', 'clavicle_l'},
	RIGHTCOLLAR={'rightcollar', 'rcollar', 'rclavicle', 'clavicle_r'},
	NECK={'neck', 'neck_01'},
}
M.fingersThesaurus={
	ThWrist={'Thumb1', 'thumb_01'}, 
	ThMetac={'Thumb2', 'thumb_02'}, -- Metacarpophalangeal joint of the right thumb
	ThIntra1={'Thumb3', 'thumb_03'},
	F1Metac= {'Index1','index_01'}, -- Metacarpophalangeal joint of the right index finger
	F1Intra1={'Index2','index_02'}, 
	F1Intra2={'Index3','index_03'}, 
	F2Metac= {'Middle1', 'middle_01'}, -- Metacarpophalangeal joint of the right middle finger 
	F2Intra1={'Middle2', 'middle_02'}, 
	F2Intra2={'Middle3', 'middle_03'}, 
	F3Metac= {'Ring1','ring_01'}, -- Metacarpophalangeal joint of the right ring finger
	F3Intra1={'Ring2','ring_02'}, 
	F3Intra2={'Ring3','ring_03'}, 
	F4Metac= {'Little1', 'Pinky1','pinky_01'}, -- Metacarpophalangeal joint of the right pinky finger
	F4Intra1={'Little2', 'Pinky2','pinky_02'}, 
	F4Intra2={'Little3', 'Pinky3','pinky_03'}, 
}
...
function M.gotoTpose(mLoader, bExcludingFingers, option_)
	if option_ then mLoader:rotateBoneGlobal(mLoader:getBoneByTreeIndex(1), quater(math.rad(-90),upv())) end
...
function M.AngleRetarget:__init(chosenFile, options)
	...
	local matchedA=boolN(motA.loader:numBone())
	local matchedB=boolN(motB.loader:numBone())
	...
	for i=2, motA.loader:numBone()-1 do
		local pi=motA.loader:bone(i):parent():treeIndex() 
		if matchedA(i)  and not matchedA(pi) then
			local ca=countUnmatchedParent(motA.loader, i, matchedA)
			local j=treeIndexBfromA[i]
			local cb=countUnmatchedParent(motB.loader, j, matchedB)

			if ca==cb then
				local boneA=motA.loader:bone(i):parent()
				local boneB=motB.loader:bone(j):parent()
				for k=1, ca do
	...
	if false then
		debugSkinA= RE.createVRMLskin(motA.loader, true)
		...
		debugSkinB= RE.createVRMLskin(motB.loader, true)
...
function M.createSkinA()
	local decodeBindPose=M.decodeBindPose
	mSkinA= RE.createVRMLskin(mSkelA, false)
	mSkinA:scale(A.skinScale, A.skinScale, A.skinScale)
