...
def onFrameChanged(iframe):
    ...
    try:
        ...
        if data and 'xsens' in data:
            ...
            if iframe < 10 :
                l.dostring("transferPose()")
            else:
                l.dostring("transferPose2()")
...
def ctor(this):
    ...
    this.create("Input", 'file_name_', 'File name (.dof)', 1)
    this.create("Button", 'export motion', 'Export Motion (.dof)',0)

    this.setWidgetHeight(100)
    this.create("Multi_Browser", "body parts", "body parts",0) # or you can instead use Select_Browser for single select.
    if True:
    ...
    skin.setMaterial('blue_transparent')
...
def onCallback(w, usrData):
    ...
    elif w.id()=='export motion':
        motionExport()

def motionExport():
    global loader, emulation
    motion_length=len(emulation.data_list)
    l.getglobal("generate_exportfile")
    l.push(motion_length)
    l.call(1,0)
    ETRI_xsens_BVH_queue = Queue()
    ETRI_xsens_BVH=XsensBVH()
    emulation.current_idx=0

    for i in range(motion_length-2):
        data, message=emulation.getOneData()
        if data and 'xsens' in data:
            ETRI_xsens_BVH_queue.put(data)
            cur_pose=ETRI_xsens_BVH.make_posedof(ETRI_xsens_BVH_queue,loader)
            loader.setPoseDOF(cur_pose)
            l.getglobal("setting_pose")
            l.push(i)
            l.call(1,0)

    file_path_=emulation.file_path
    l.getglobal("export_motfile")
    l.push(file_path_)
    l.call(1,0)
    emulation.current_idx=0

def main():
    scriptFile="gym_mp/realtimePose2.lua"
