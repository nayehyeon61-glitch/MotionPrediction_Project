import os.path
import struct
import numpy as np
import socket
import pandas as pd
from datetime import datetime

import queue
import csv


delsys_column = np.dtype([
    ('sensor_1', 'f4'),
    ('sensor_2', 'f4'),
    ('sensor_3', 'f4'),
    ('sensor_4', 'f4'),
    ('sensor_5', 'f4'),
    ('sensor_6', 'f4'),
    ('sensor_7', 'f4'),
    ('sensor_8', 'f4'),
    ('sensor_9', 'f4'),
    ('sensor_10', 'f4'),
    ('sensor_11', 'f4'),
    ('sensor_12', 'f4'),
    ('sensor_13', 'f4'),
    ('sensor_14', 'f4'),
    ('sensor_15', 'f4'),
    ('sensor_16', 'f4')
])
        
class delsys_120_csv:    
    def make_csv(self, data, timestamp, click_timestamp):
        convert_timestamp = datetime.strptime(click_timestamp, "%Y-%m-%d %H:%M:%S")
        delsys_arry = []
        count_id = 0
        for i in range(len(data)):
            sensor_data = data[i][1]
            delsys_arry.append(sensor_data)
            #count_id += 1
            
        df = pd.DataFrame(delsys_arry)
        df_flat = pd.DataFrame([df.values.flatten()])
        df_flat.columns = delsys_column.names
        
        dir_time = convert_timestamp.strftime("Etri_SoftSuit_Dataset_%Y_%m_%d_%H_%M_%S")
        dir_path = "./dataset/Etri_SoftSuit_Dataset/{}".format(dir_time)
        os.makedirs(dir_path, exist_ok=True)
        
        timestamp_set = convert_timestamp.strftime("%Y_%m_%d_%H_%M_%S")
        #self.file_name = "dataset/Etri_SoftSuit_Dataset/DelsysEMG_{}.csv".format(timestamp_set)
        self.file_name = r"{}/DelsysEMG_{}.csv".format(dir_path, timestamp_set)
        
        ts_df = pd.DataFrame([timestamp], columns=["Timestamp"])
        output_df = pd.concat([ts_df, df_flat], axis=1)
        
        if not self.file_exist():
            output_df.to_csv(self.file_name, mode='a', index=False, header=True)
        else:
            output_df.to_csv(self.file_name, mode='a', index=False, header=False)
    
    def make_csv_v2(self, result_list, click_timestamp):
        while len(result_list) > 0:
            data = result_list[0]['delsys']
            timestamp = struct.unpack('!32s', result_list[0]["TS"])[0].decode()
            result_list.pop(0)
            
            convert_timestamp = datetime.strptime(click_timestamp, "%Y-%m-%d %H:%M:%S")
            delsys_arry = []
            count_id = 0
            for i in range(len(data)):
                sensor_data = data[i][1]
                delsys_arry.append(sensor_data)
                #count_id += 1
                
            df = pd.DataFrame(delsys_arry)
            df_flat = pd.DataFrame([df.values.flatten()])
            df_flat.columns = delsys_column.names
            
            dir_time = convert_timestamp.strftime("Etri_SoftSuit_Dataset_%Y_%m_%d_%H_%M_%S")
            dir_path = "./dataset/Etri_SoftSuit_Dataset/{}".format(dir_time)
            os.makedirs(dir_path, exist_ok=True)
            
            timestamp_set = convert_timestamp.strftime("%Y_%m_%d_%H_%M_%S")
            #self.file_name = "dataset/Etri_SoftSuit_Dataset/DelsysEMG_{}.csv".format(timestamp_set)
            self.file_name = r"{}/DelsysEMG_{}.csv".format(dir_path, timestamp_set)
            
            ts_df = pd.DataFrame([timestamp], columns=["Timestamp"])
            output_df = pd.concat([ts_df, df_flat], axis=1)
            
            if not self.file_exist():
                output_df.to_csv(self.file_name, mode='a', index=False, header=True)
            else:
                output_df.to_csv(self.file_name, mode='a', index=False, header=False)
        print("delsys CSV Done : ", datetime.now())
        
    def make_csv_v3(self, delsys_queue, click_timestamp):
        while True :
            queue_data = delsys_queue.get()
            
            if queue_data == True:
                break
            
            if 'delsys' in queue_data:
                data = queue_data['delsys']
                data_TS = queue_data['TS']
                
            
                timestamp = struct.unpack('!32s', data_TS)[0].decode()
                
                
                convert_timestamp = datetime.strptime(click_timestamp, "%Y-%m-%d %H:%M:%S")
                delsys_arry = []
                count_id = 0
                for i in range(len(data)):
                    sensor_data = data[i][1]
                    delsys_arry.append(sensor_data)
                    #count_id += 1
                    
                df = pd.DataFrame(delsys_arry)
                df_flat = pd.DataFrame([df.values.flatten()])
                df_flat.columns = delsys_column.names
                
                dir_time = convert_timestamp.strftime("Etri_SoftSuit_Dataset_%Y_%m_%d_%H_%M_%S")
                dir_path = "./dataset/Etri_SoftSuit_Dataset/{}".format(dir_time)
                os.makedirs(dir_path, exist_ok=True)
                
                timestamp_set = convert_timestamp.strftime("%Y_%m_%d_%H_%M_%S")
                #self.file_name = "dataset/Etri_SoftSuit_Dataset/DelsysEMG_{}.csv".format(timestamp_set)
                self.file_name = r"{}/DelsysEMG_{}.csv".format(dir_path, timestamp_set)
                
                ts_df = pd.DataFrame([timestamp], columns=["Timestamp"])
                output_df = pd.concat([ts_df, df_flat], axis=1)
                
                if not self.file_exist():
                    output_df.to_csv(self.file_name, mode='a', index=False, header=True)
                else:
                    output_df.to_csv(self.file_name, mode='a', index=False, header=False)
        print("[CSV] delsys CSV Done : ", datetime.now())
    
    def file_exist(self):
        # path = r"C:\Users\ADMIN\Desktop\soo\git\HIL-code\Examples\dataset\ETRI SoftSuit dataset\%s" % self.file_name
        path = self.file_name
        self.isExist = os.path.exists(path)

        return self.isExist