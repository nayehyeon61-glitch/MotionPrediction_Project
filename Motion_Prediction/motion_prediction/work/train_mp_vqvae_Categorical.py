...
from gym_mp.models_vqvae import GumbelSoftmaxVQVAE, Model3
...
def main():
    with open("config/vqvaecategorycal.json", "r") as file:
    ...
    if torch.isnan(mocap_data).any():
        ...
        mocap_data = torch.where(torch.isnan(mocap_data), torch.tensor(0.0), mocap_data)
    ...
    vqvae = Model3(input_size, input_frames, 1024, 8, 128).to(device)
    ...
    scheduler_lr = optim.lr_scheduler.StepLR(vqvae_optimizer,step_size=5,gamma=0.9)
    #scheduler_lr = optim.lr_scheduler.CyclicLR(vqvae_optimizer,initial_lr/100,initial_lr,20)
    ...
    if train:
        ...
        for ep in range(1,num_epochs+1):
            ...
            for num_of_minibatch,indices in enumerate(sampler):
                ...
                for i in range(1, input_frames):
                    out_range = torch.cat((out_range,out_range[:,-1].unsqueeze(1)+1),dim=1)
                    
                encode_buffer = mocap_data[in_range]
                gt_buffer = mocap_data[out_range]
                knn=torch.ones(1, device=device)
                
                curr_pose = encode_buffer.flatten(start_dim=1, end_dim=2)
                gt_pose = gt_buffer.flatten(1,2)
                
                
                x_hat, loss, recon_loss, hidden = vqvae(curr_pose, knn,gt_pose)
                ...
                for i in range (1, 1):
                    in_range = torch.cat((in_range, in_range[:,-1].unsqueeze(1)+1),dim=1)
                    out_range = torch.cat((out_range,out_range[:,-1].unsqueeze(1)+1),dim=1)
                    encode_buffer = mocap_data[in_range]
                    gt_buffer = mocap_data[out_range]
                    
                    curr_pose = encode_buffer.flatten(start_dim=1, end_dim=2)[:,35:]
                    gt_pose = gt_buffer.flatten(1,2)[:,35:]
                    
                    x_hat, loss, recon_loss, hidden = vqvae(curr_pose, knn,gt_pose, hidden)
                    vqvae_optimizer.zero_grad()
                    (recon_loss+loss).backward(retain_graph=True)
                    vqvae_optimizer.step()
        ...
        settings.VQVAELTO = torch.load(pt_path).to(device)
        ...
        #codebook = settings.VQVAELTO.VQBottom.embedding.weight
...
def test_vqvaelto(input, conditionfr, vqvae_output, hidden_gru):
    with torch.no_grad():
        settings.VQVAELTO.eval()
        
        conditionfr = torch.tensor(Tonumpy(conditionfr)).float()
        hidden = torch.tensor(Tonumpy(hidden_gru)).float()
        hidden = hidden.view([2,30,1024])
        ...
        knn=torch.ones(1)
        #_, output = settings.VQVAELTO(real_inp, condition, 1)
        output, newHidden = settings.VQVAELTO(real_inp, knn, hidden=hidden)
        
        newHidden = newHidden.view(-1)
        ...
        return vqvae_output, hidden_gru
