...
def lua_step(frameNum,inputdata):
    ...
    actionvec.ref()[:]=inputdata
    l.push(frameNum)
    l.push(actionvec)
    #under 3 line is only one action (ex. swing)
    #inputAction=np.float64(inputdata)
    #l.push(inputAction)
    #if train situation  
    #inputAction=np.float64(inputdata[0])
    #l.push(inputAction)
    #else replay situation
    #l.push(2.0)
    l.call(2,3)
...
#vae code
def lua_vae_step(output_pose,frame_num):
    l=mlib.getPythonWin()
    if not l.isLuaReady() : return
    l.getglobal("vae_step")
    output_pose_vecn=m.vectorn()
    output_pose_vecn.setSize(len(output_pose[0,0]))
    output_pose_vecn.ref()[:]=output_pose[0,0]
    l.push(output_pose_vecn)
    l.push(frame_num[0].item())
    l.call(2,0)
