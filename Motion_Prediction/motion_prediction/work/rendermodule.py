...
def fileChooser(*args):
    file_name=lua.F(("Fltk","chooseFile"),'choose a motion file', '../python/ETRI_viewer/' ,'*.bin',False)
    return file_name
...
def BVHloader(filename, option=None):
    if filename[-4:]=='.bvh' and option==None:
        loader=m.MotionLoader._createm(filename)
        doNotGarbageCollect.append(loader)
        return loader

    var_name='mLoader'+m.generateUniqueName()
    lua.F_lua(var_name, "MainLib.BVHLoader", filename, option)
    info={} # todo : return this when requested through an option
    info['var_name']=var_name
    return lua.G_MotionLoader(var_name)
...
def createMainWin(argv):
    ...
    options=[]
    scriptFile=[]
    useSeperateWin=False
    dostring=None
    for i in range(1, len(argv)):
        if argv[i][0:2]=='--':
            if argv[i]=='--sep':
                useSeperateWin=True
            elif option[0:10]=='--dostring':
                dostring=option[11:]
            else:
