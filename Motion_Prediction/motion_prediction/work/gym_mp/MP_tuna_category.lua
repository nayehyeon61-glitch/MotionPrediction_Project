...
-- type1 : 35(DOF), type2 : 57(Quat) type3 : 81 (6dim)
...
function mode_setting(mode_)
	...
	historypose = matrixn(10, model_config.pose_size)
	historypose:setAllValue(0)
end
...
function EVR:onFrameChanged(win, iframe)
	...
	if iframe < 40 then 
		...
		pushBack(historypose, mMotionDOF:row(iframe + 10))
	else 
		
		prev_pose:assign(mMotionDOF:row(iframe+9))
		inputBuffer:setAllValue(0)

		-- encode input
		for i = 1, model_config.input_frames do
			pushBack(inputBuffer, poseEncodeUpdate(iframe-model_config.input_frames+i-1)) -- [오래된거 ...  최신꺼] pushBack 용이하게.
		end
		...
		python.F('train_mp_vqvae_Categorical','test_vqvaelto',inputBuffer:toVector(),inputBuffer:row(0),encodeData, hidden_gru)
		
		pose:assign(poseDecodeUpdate(encodeData,mMotionDOF:row(iframe+9)))
		pose:range(0,7):assign(mMotionDOF:row(iframe+10):range(0,7))
		-- filter process 
		...
		if not model_config.ALIGN then
			pp_pose:setVec3(0,mMotionDOF:row(iframe+10):toVector3(0))
			-- for i=1, 10  do
			-- 	historypose:row(i-1):setVec3(0, mMotionDOF:row(iframe+i):toVector3(0))
			-- end
		else
			pp_pose:setVec3(0,mMotionDOF:row(iframe):toVector3(0))
			for i=1, 10  do
				historypose:row(i-1):setVec3(0, mMotionDOF:row(iframe):toVector3(0))
			end
		end
		pushBack(historypose, pp_pose)
		for i=1, 10  do
			mSkin_[i]:setPoseDOF(historypose:row(i-1))
		end
		
		prev_pose:assign(pp_pose)
		mLoader_predictied:setPoseDOF(pp_pose)
		if model_config.ATTACH_CAMERA  then
		...
		--ground truth pose
		mSkin2:setPoseDOF(mMotionDOF:row(iframe+model_config.prediction_frame-1))
...
function All_data_frames()
	...
	elseif model_config.motionDataType==3 then
		for i=1,data:rows() do
			data:row(i-1):assign(poseEncodeUpdate6Dim(i-1))
		end
	end
	return data
end
...
function poseEncodeUpdate(iframe)
	...
	local quat = pose1:toTransf(0).rotation
    local dv = quat:inverse()*(pose2:toVector3(0) - pose1:toVector3(0))*30
	local rotmat = rotationMatrix(quat:inverse()*pose2:toQuater(3))
...
function poseEncodeUpdate_old(iframe)
    local vec = vectorn(model_config.update_encode_size)
    local pose1 = mMotionDOF:row(iframe)
    local pose2 = mMotionDOF:row(iframe+1)
	local rotY = pose1:toTransf(0):project2D().rotation:rotationY()
...
function poseEncodeUpdateQuater(iframe)
	...
	vec:setVec3(3,Y)
	vec:setVec3(6,Z)
	local d = getLocalTransfFromLoader_fromparents(iframe+1, false)
	for i = 1, 12 do
...
function poseEncodeUpdate6Dim(iframe)
	mLoader1:setPoseDOF(mMotionDOF:row(iframe))
    local vec = vectorn(model_config.update_encode_size)
    local pose1 = mMotionDOF:row(iframe)
    local pose2 = mMotionDOF:row(iframe+1)
	local rotY = pose1:toTransf(0):project2D().rotation:rotationY()
	local quat = pose1:toTransf(0).rotation
	local beforetransf = mLoader1:getBoneByTreeIndex(1):getFrame()
    local dv = quat:inverse()*(pose2:toVector3(0) - pose1:toVector3(0))*30
	local rotmat = rotationMatrix(quat:inverse()*pose2:toQuater(3))
	mLoader1:setPoseDOF(mMotionDOF:row(iframe+1))
	local Y = rotmat:getColumn(1)
	local Z = rotmat:getColumn(2)
	...
	for i = 1, 12 do
		local local_transf = beforetransf.rotation:inverse()*mLoader1:getBoneByTreeIndex(i+1):getFrame().rotation
		local local_rotmat = rotationMatrix(local_transf)
		local bone_Y = local_rotmat:getColumn(1)
		local bone_Z = local_rotmat:getColumn(2)
		local bone_X = vector3.cross(bone_Y, bone_Z)

		if false then -- just for testcode
			local test = quater()
			test:setFrameAxesYZ(bone_Y, bone_Z)
			local test2 = beforetransf.rotation*test
			local loss = test2 - mLoader1:getBoneByTreeIndex(i+1):getFrame().rotation
			local loss2 = loss.x + loss.y + loss.z + loss.w 
			if loss2 > 0.00001 then
				print(" over loss")
				print(loss2)
			end
		...
		vec:setVec3(3 +i*6, bone_Y)
		vec:setVec3(6 + i*6, bone_Z)
	end
	return vec
end

-- poseEncodeEnd
function getLocalTransfFromLoader_fromparents(iframe, draw)
	mLoader1:setPoseDOF(mMotionDOF:row(iframe))
	...
	for i = 2, bonenum-1 do
		--local transform = mLoader1:getBoneByTreeIndex(i):parent():getFrame():inverse()*mLoader1:getBoneByTreeIndex(i):getFrame()
		local transform = rootbonetransf:inverse()*mLoader1:getBoneByTreeIndex(i):getFrame()

		pos(i):assign(transform.translation)
		ori(i):assign(transform.rotation)
		if draw then 
...
function poseDecodeUpdateQuater(encodeData,prev_pose)
	...
	local t = 7
	for i = 2, 13 do
		local q = encodeData:toQuater(5+4*(i-1))
...
function poseDecodeUpdate6Dim(encodeData,prev_pose)
	local pose_output = vectorn(33)
	pose_output:setAllValue(0)
	local prev_root_pos = prev_pose:toVector3(0)
	local prev_root_ori = prev_pose:toTransf(0):project2D().rotation
	...
	mLoader1:setPoseDOF(prev_pose)
	local root_rot = prev_pose:toTransf(0)

	local t = 7
	for i = 2, 13 do
		local y = encodeData:toVector3(3+6*(i-1))
		local z = encodeData:toVector3(6+6*(i-1))
		local q = quater()
		q:setFrameAxesYZ(y, z)
		local newq = mLoader1:getBoneByTreeIndex(1):getFrame().rotation*q
		newq = mLoader1:getBoneByTreeIndex(i):parent():getFrame():inverse().rotation*newq
		local bone = mLoader1:getBoneByTreeIndex(i)
		local rochannel = bone:getRotationalChannels()
		local rot = newq:getRotation(rochannel)
		local vrot = vectorn(3)
...
function poseDecodeUpdate_old(encodeData,prev_pose)
	local pose_output = vectorn(33)
	local prev_root_pos = prev_pose:toVector3(0)
	local prev_root_ori = prev_pose:toTransf(0):project2D().rotation
	local rotY = prev_root_ori:rotationY()
...
function onCallback(w, userData)
	...
	elseif w:id() == "seperate from ground truth" then
		...
		for i=1,model_config.prediction_frame do 
			...
			mSkin2:setTranslation(2*dist,0,0)
		end
...
function eval_prediction_error2(loader_sim,loader_ref)
	local num_bone=0;
	num_bone=loader_sim:numBone()-1
	assert(loader_sim:numBone()==loader_ref:numBone())
	...
	print('mpre(redian)',mpjre_radian)
	print('--------------------------')
	return mpjpe, mpjre, mpjre_radian
end
