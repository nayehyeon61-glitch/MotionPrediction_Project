...
function ctor()
	...
	encodeBuffer = matrixn(2,35)
	poseBuffer = matrixn(9,35)
...
function All_data_frames()
	local data = matrixn(mMotionDOF:numFrames()-1,mMotionDOF:numDOF()-7)
	for i=0,data:rows()-1 do 
		data:row(i):assign(mMotionDOF:row(i):range(7,33))
	end
	return data
end
-- 89 --> 26 dof 
