...
function mode_setting(mode_)
	...
	if mode_==0 then
		...
		--train == 1
		datapath="../Resource/motion/prediction/stitchNewMotion_hyunwoo.dof"
...
function EVR:onFrameChanged(win, iframe)
	...
	if iframe < 15 then 
		...
		for i=1,model_config.prediction_frame  do 
			...
			-- run model
			python.F('train_mp_vavae','test_vqvaelto',inputBuffer:toVector(),inputBuffer:row(model_config.prediction_frame-1),encodeData)
