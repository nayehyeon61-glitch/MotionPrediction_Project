...
function EVR:onFrameChanged(win, iframe)
	...
	if iframe < 10 then 
		...
		for i = 1, model_config.input_frames do
			...
			pushBack(inputBuffer, poseEncodeUpdate(iframe+i-5)) -- 1 frame to 10 frame pushBack
		...
		for i=1,model_config.prediction_frame  do 
		    ...
		    mSkin_[i]:setTranslation(-100,-5,0)
			...
			if model_config.ATTACH_CAMERA and i==model_config.prediction_frame then
	    		...
	    		RE.viewpoint().vat:set(renderPoseFiltered:toVector3(0).x*100,100,renderPoseFiltered:toVector3(0).z*100)
				RE.viewpoint().vpos:set(renderPoseFiltered:toVector3(0).x*100 , 100 ,renderPoseFiltered:toVector3(0).z*100+400 )
				RE.viewpoint():update()
			end
		end
	...
	for i=0,data:rows()-1 do
        ...
        latent:setAllValue(0)
			...
			--python.F('train_motionprediction_dif16','test_dif1016',inputBuffer:row(0),inputBuffer:row(1),inputBuffer:row(2),inputBuffer:row(3),inputBuffer:row(4),inputBuffer:row(5),inputBuffer:row(6),inputBuffer:row(7),inputBuffer:row(8),inputBuffer:row(9),latent,encodeData)
				...
				for i=1,3 do
				python.F('train_motionprediction_dif_multi_n_attn','test_noise_output',inputBuffer:row(0),inputBuffer:row(1),inputBuffer:row(2),inputBuffer:row(3),inputBuffer:row(4),inputBuffer:row(5),inputBuffer:row(6),inputBuffer:row(7),inputBuffer:row(8),inputBuffer:row(9),latent,encodeData)
                tempData=encodeData+tempData
				end
				tempData=tempData/3
