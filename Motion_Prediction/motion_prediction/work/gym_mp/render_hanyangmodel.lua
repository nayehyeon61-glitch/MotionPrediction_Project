...
function prepareEnv(source_loader, source_skin)
	...
	mTargetSkin:setTranslation(0,0,0)

    loader_prediction=MainLib.VRMLloader ("../Resource/motion/prediction/hyunwoo_lowdof_T_boxfoot.wrl")
	datapath=Fltk.chooseFile('choose a motion (Hanyang)', '../Resource/motion/prediction/vqvaedataset/' ,'*.dof', false)
	while datapath:len()==0 do
		datapath=Fltk.chooseFile('choose a motion (Hanyang)', '../Resource/motion/prediction/vqvaedataset/' ,'*.dof', false)
		if datapath:len()>0 then
			break;
		end
	end
    ...
    DOFcontainer = MotionDOFcontainer(loader_prediction.dofInfo ,datapath)
	mMotionDOF = DOFcontainer.mot

    mskin_hanyang=RE.createVRMLskin(loader_prediction , false)
	mskin_hanyang:scale(config.skinScale,config.skinScale,config.skinScale)
	mskin_hanyang:setMaterial('red_transparent')
	mskin_hanyang:setVisible(true)
	mskin_hanyang:setTranslation(100,0,0)
...
--------from here, posetransfer functions--------
function setTpose_enforce()
	local M=require("subRoutines/retarget_common")
	dbg.line()
	print('set bone dictionary: Original model (ETRI)')
	...
	do 
		...
		local transfer_pose=vectorn()
		mTargetLoader:getPoseDOF(transfer_pose)
		local gg_=MotionDOFcontainer(mTargetLoader.dofInfo, transfer_pose)
	...
	dbg.line()
	print('set bone dictionary: Target model (Hanyang)')
	...
	if false then
		...
		if true then
			--toe
			local post_pose=vectorn()
			mTargetLoader:getPoseDOF(post_pose)
			post_pose:set(source_redundant[1],0.4)
			post_pose:set(source_redundant[2],0.4)
			mTargetLoader:setPoseDOF(post_pose)
...
function transferPose()
	...
	if globalframe<mMotionDOF:rows()-2 then
		mskin_hanyang:setPoseDOF(mMotionDOF:row(globalframe))
	end
	globalframe = globalframe+1
end
--------until here, posetransfer functions--------
