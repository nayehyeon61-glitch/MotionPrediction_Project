...
function mode_setting(mode_)
	...
	if mode_==0 then
		...
		--train == 1
		-- datapath="../Resource/motion/prediction/vqvaedataset/Train_dataset/stitchNewMotion_hyunwoo.dof"
...
function EVR:onFrameChanged(win, iframe)
	...
	if iframe < 15 then 
		...
		for i=1,model_config.prediction_frame  do 
			...
			-- run model
			python.F('train_mp_vqvae_CNN','test_vqvaelto',inputBuffer:toVector(),inputBuffer:row(model_config.prediction_frame-1),encodeData)


			pushBack(inputBuffer, encodeData)
...
function poseEncodeUpdate(iframe)
    ...
    local dv = quat:inverse()*(pose2:toVector3(0) - pose1:toVector3(0))
