...
function MainLib.VRMLloader:convertToConvexes()
	local loader=self
...
function OsimParser:__init(filename, options)
	...
	local function rectifyMuscle(mscl)
		...
		mscl.GeometryPath.PathPointSet=mscl.GeometryPath.PathPointSet.objects.PathPoint
		if mscl.GeometryPath.PathPointSet then
			for i, v in ipairs(mscl.GeometryPath.PathPointSet) do
				...
				v.location_wrl={v.location.x, v.location.y, v.location.z}
