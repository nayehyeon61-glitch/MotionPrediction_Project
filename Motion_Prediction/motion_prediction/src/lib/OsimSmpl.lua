...
function out.gotoSMPL_Tpose(osimLoader)
	local pose=osimLoader:pose()
	local height=parser.loader:getBoneByName('toes_l'):getFrame().translation.y
...
function out.findBetas(dd, osimLoader)
...
function out.alignPoses(dd, osimLoader)
	local function genPair(n1, n2, offset)
		return { array.locate(SMPL.SMPL_JOINT_NAMES, n1)-1, osimLoader:getTreeIndexByName(n2), offset }
	end
	local alignInfo={
