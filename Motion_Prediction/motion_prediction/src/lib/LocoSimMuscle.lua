...
function OnlineLocoSynthesis:setFloor(sim)
	...
	mesh:initBox(vector3(380, 0.2, 380))
...
--	self.simulator:createObstacle(mesh)
	...
	self.pdservo=QPservo2(self.skel_withoutCOMjoint.dofInfo, timestep, Physics.DynamicsSimulator.EULER, self.simulator, model) 
...
function OnlineLocoSynthesis:oneStepSimul()   
	...
	if simulStart>=0 then   
		...
		for currSimulFrame=simulStart,simulEnd-1 do	 
			...
			if mrdMotion then
				mrd_info.avgTheta:radd(self.simulator:getLastSimulatedPose(0))
			end
	...
	if self.errorOccurred or error_feedback_method==EFM.NONE then 
		...
		do
			...
			if true then
				...
				local ctf=MotionDOF.rootTransformation(sim:getLastSimulatedPose(0))
			...
			local dbgPose
			if true then
				mm=sim:calcMomentumCOMfromPose(0, delta_t, sim:getLastSimulatedPose(0), examplePose) sign=-1.0
				--mm=sim:calcMomentumCOMfromPose(0, delta_t, examplePose, sim._lastSimulatedPose)
			else
				local pose2=vectorn()
				pose2:interpolate(delta_t, examplePose,sim:getLastSimulatedPose(0))
				local rootOri2=quater()
				rootOri2:interpolate(delta_t,examplePose:toQuater(3),sim:getLastSimulatedPose(0):toQuater(3))
			...
			local I=vectorn()
			sim:calcInertia(0, sim:getLastSimulatedPose(0),I);
