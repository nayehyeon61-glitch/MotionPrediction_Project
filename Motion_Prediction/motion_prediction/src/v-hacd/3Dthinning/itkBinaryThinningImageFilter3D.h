...
#include <BaseLib/image/Image3d.h>
...
class BinaryThinningImageFilter3D
{
			...
			inputImage(NULL)
	{
		...
		void createImage(int x, int y, int z)
		{
			inputImage=new Image3D(x,y,z, (byte)0);
			width=x;
			height=y;
			depth=z;
		}

		void set(int x, int y, int z, unsigned int value) { setPixel(*inputImage, x, y, z, (byte)value);}
		unsigned int get(int x, int y, int z) { return getPixel(*inputImage, x,y,z);}

		int countNeighborhood(int x, int y, int z) {


			int numberOfNeighbors = -1;   // -1 and not 0 because the center pixel will be counted as well
			byte neighbor [27];
			getNeighborhood(neighbor, *inputImage, x, y, z);
		...
		{
			// Prepare data
			prepareData(*inputImage);

			// Compute Thinning	
			computeThinImage(*inputImage);
		 ...
		 */
		void prepareData(Image3D & outputImage) 
		{
			...
			for (int z = 0; z < depth; z++) 
				for (int x = 0; x < width; x++) 
					for (int y = 0; y < height; y++)
						if ( inputImage->getPixels(z)[x + y * width] != 0 )
							( outputImage.getPixels(z))[x + y * width] = 1;
		...
		void computeThinImage(Image3D & outputImage) 
		{
			...
			std::vector<Int3D> simpleBorderPoints ;
			...
			{						
				...
				for( int currentBorder = 1; currentBorder <= 6; currentBorder++)
				{
					...
					for (int z = 0; z < depth; z++)
					{
						for (int y = 0; y < height; y++)
						{
							for (int x = 0; x < width; x++)						
							{
								...
								byte neighborhood [27];
								getNeighborhood(neighborhood, outputImage, x, y, z);
					...
					for (int isbp=0; isbp<simpleBorderPoints.size(); isbp++)
					{
						index = simpleBorderPoints[isbp];

						byte neighborhood [27];
						getNeighborhood(neighborhood, outputImage,index[0], index[1], index[2]);
		 ...
		 */
		bool isEndPoint(Image3D & image, int x, int y, int z)
		{
			int numberOfNeighbors = -1;   // -1 and not 0 because the center pixel will be counted as well
			byte neighbor [27];
			getNeighborhood(neighbor, image, x, y, z);
			for( int i = 0; i < 27; i++ ) // i =  0..26
			{					        	
				if( neighbor[i] == 1 )
					numberOfNeighbors++;
		 ...
		 */
		inline byte getPixel(Image3D const & image, int x, int y, int z)
		{
			if(x >= 0 && x < this->width && y >= 0 && y < this->height && z >= 0 && z < this->depth)
				return ( image.getPixels(z))[x + y * this->width];
			else return 0;
		} /* end getPixel */
		/* -----------------------------------------------------------------------*/
		/**
		 ...
		 * 
		 * @param image 3D image (Image3D)
		 * @param x x- coordinate
		 * @param y y- coordinate
		 * @param z z- coordinate (in image stacks the indexes start at 1)
		 * @return corresponding 27-pixels neighborhood (0 if out of image)
		 */
		void getNeighborhood(byte* neighborhood, Image3D & image, int x, int y, int z)
		{
		 ...
		 */
		inline byte getPixelNoCheck(Image3D const& image, int x, int y, int z)
		{		
			return ( image.getPixels(z))[x + y * this->width];		
		} /* end getPixelNocheck */
		 ...
		 */
		void setPixel(Image3D & image, int x, int y, int z, byte value)
		{
			if(x >= 0 && x < this->width && y >= 0 && y < this->height && z >= 0 && z < this->depth)
				image.getPixels(z)[x + y * this->width] = value;
		} /* end setPixel */
		 ...
		 */
		inline byte N(Image3D const& image, int x, int y, int z)
		{
		 ...
		 */
		inline byte S(Image3D const& image, int x, int y, int z)
		{
			return getPixel(image, x, y+1, z);
		} /* end S */
		 ...
		 */
		inline byte E(Image3D const& image, int x, int y, int z)
		{
		 ...
		 */
		inline byte W(Image3D const& image, int x, int y, int z)
		{
			return getPixel(image, x-1, y, z);
		} /* end W */
		 ...
		 */
		inline byte U(Image3D const& image, int x, int y, int z)
		{
		 ...
		 */
		inline byte B(Image3D const& image, int x, int y, int z)
		{
			return getPixel(image, x, y, z-1);
		} /* end B */
		 ...
		 */
		bool  isEulerInvariant(byte* neighbors, int * LUT)
		{
		...
		inline unsigned char indexOctantNEB(byte* neighbors) {
			unsigned char n;
			n = 1;
			if( neighbors[2]==1 )
				n |= 128;
		...
		inline unsigned char indexOctantNWB(byte* neighbors) {
			unsigned char n;
			n = 1;
			if( neighbors[0]==1 )
				n |= 128;
		...
		inline unsigned char indextOctantSEB(byte* neighbors) {
			unsigned char n;
			n = 1;
			if( neighbors[8]==1 )
				n |= 128;
		...
		inline unsigned char indexOctantSWB(byte* neighbors) {
			unsigned char n;
			n = 1;
			if( neighbors[6]==1 )
				n |= 128;
		...
		inline unsigned char indexOctantNEU(byte* neighbors) {
			unsigned char n;
			n = 1;
			if( neighbors[20]==1 )
				n |= 128;
		...
		inline unsigned char indexOctantNWU(byte* neighbors) {
			unsigned char n;
			n = 1;
			if( neighbors[18]==1 )
				n |= 128;
		...
		inline unsigned char indexOctantSEU(byte* neighbors) {
			unsigned char n;
			n = 1;
			if( neighbors[26]==1 )
				n |= 128;
		...
		inline unsigned char indexOctantSWU(byte* neighbors) {
			unsigned char n;
			n = 1;
			if( neighbors[24]==1 )
				n |= 128;
		 ...
		 */
		bool isSimplePoint(byte* neighbors) 
		{
